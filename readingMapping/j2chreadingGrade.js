...
for (const [index, kanji] of data.characters.entries()) {
	...
	for (const reading of kanji.readingMeaning.groups[0].readings) {
		...
		if (reading.type === 'pinyin') {
			...
			if (pinyin.includes(' ')) {
				...
				// make array of objects with pinyin and kanji for each pinyin in pinyin split
				const pinyinSplitObjects = pinyinSplit.map(pinyin => ({ pinyin, kanji: kanji.literal }));

				pinyinReadings.push(...pinyinSplitObjects);
				continue;
			}

			pinyinReadings.push({ pinyin, kanji: kanji.literal });
		}
	...
	// 	if (!mapOnToPinyin[pinyinReading]) {
	// 		mapOnToPinyin[pinyinReading] = [];
	// 	}

	// 	mapOnToPinyin[pinyinReading].push(...onReadings);
	// }
	// map on readings to pinyin readings
	for (const onReading of onReadings) {
		if (!mapOnToPinyin[onReading]) {
			mapOnToPinyin[onReading] = [];
		}

		mapOnToPinyin[onReading].push(...pinyinReadings);
	}
}

console.log(mapOnToPinyin);

// // now we calculate the percentage of the time that a pinyin reading maps to an on reading
// const mapOnToPinyinPercentages = {};

// // condense mapOnToPinyin to a dictionary of pinyin readings to a dictionary of on readings to percentages and kanji



// for (const pinyinReading in mapOnToPinyin) {
// 	const onReadings = mapOnToPinyin[pinyinReading];
...
// 	mapOnToPinyinPercentages[pinyinReading] = frequencies;
// }

// console.log(mapOnToPinyin);
// console.log(mapOnToPinyinPercentages);

// const result = {};

// for (const entry of Object.entries(mapOnToPinyin)) {
...
function processMap(dataMap) {
	...
	for (let onyomi in dataMap) {
		const pinyinFrequencies = {};
		const pinyinKanjis = {};
		const totalCount = dataMap[onyomi].length;

		for (let pinyinObj of dataMap[onyomi]) {
			const pinyin = pinyinObj.pinyin;
			const kanji = pinyinObj.kanji;


			// Increment the frequency of the pinyin reading
			pinyinFrequencies[pinyin] = (pinyinFrequencies[pinyin] || 0) + 1;

			// Collate the kanji characters for the pinyin reading
			if (!pinyinKanjis[pinyin]) {
				pinyinKanjis[pinyin] = new Set();
			}
			pinyinKanjis[pinyin].add(kanji);
		}

		result[onyomi] = {};

		for (let pinyin in pinyinFrequencies) {
			result[onyomi][pinyin] = {
				'percentage': (pinyinFrequencies[pinyin] / totalCount) * 100,
				'kanjis': [...pinyinKanjis[pinyin]]
			};
		}
	}
...
const processedMap = processMap(mapOnToPinyin);
// console.log(processMap(mapOnToPinyin));
console.log(util.inspect(processedMap, { depth: null, colors: true }));
fs.writeFileSync('j2chReadings.json', JSON.stringify(processedMap, null, 2), 'utf8');
...
fs.writeFileSync('j2ch1.html', html, 'utf8');
